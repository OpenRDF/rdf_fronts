package com.openrdf.utils.enums;
...
public enum HTTPStatusCode {
	 ...
	 */
	NON_AUTHORIATATIVE_INFORMATION(203, "Non-Authoritative Information"),
	 ...
	 * 假如本响应请求使用了If-Range强缓存验证，那么本次响应不应该包含其他实体头；假如本响应的请求使用了If-Range弱缓存验证，那么本次响应禁止包含其他实体头；这避免了缓存的实体内容和更新了的实体头信息之间的不一致。
	 ...
	 * 没有信息能够告诉用户这个状况到底是暂时的还是永久的。假如服务器知道情况的话，
	 ...
	 * 代理服务器必须返回一个Proxy-Authenticate用以进行身份询问。
	 * 客户端可以返回一个Proxy-Authorization信息头用以验证。参见RFC 2617。
	 */
	 ...
	 * 冲突通常发生于对PUT请求的处理中。
	 * 例如，在采用版本检查的环境下，某次PUT提交的对特定资源的修改请求所附带的版本信息与之前的某个（第三方）请求向冲突，
	 ...
	 * 服务器也应当在返回416状态码的同时，包含一个Content-Range实体头，用以指明当前资源的长度。
	 * 这个响应也被禁止使用multipart/byteranges作为其Content-Type。
	 */
	REQUESTED_RANGE_NOT_STATISFIABLE(416, "Requested Range Not Satisfiable"),
	
	/**
	 * 在请求头Expect中指定的预期内容无法被服务器满足，或者这个服务器是一个代理服务器，
	 * 它有明显的证据证明在当前路由的下一个节点上，Expect的内容无法被满足。
	 */
	 ...
	 * 通常，这里的IP地址指的是从服务器上看到的客户端地址（比如用户的网关或者代理服务器地址）。
	 * 在这种情况下，连接数的计算可能涉及到不止一个终端用户。
	 */
	THERE_ARE_TOO_MANY_CONNECTIONS_FROM_YOUR_INTERNET_ADDRESS(421, "There are too many connections from your internet address"),
	 ...
	 */
	UNORDEREN_COLLECTION(425, "Unordered Collection"),
